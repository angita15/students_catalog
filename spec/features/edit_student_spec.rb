require "rails_helper"

feature "Edit student" do
	scenario "successfully" do
		visit "/teachers/sign_in"

		click_link "Sign up"

			fill_in "Email", with: "edwin@hotmail.com"
			fill_in "Password", with: "1992"
			fill_in "Password confirmation", with: "1992"

		click_on "Sign up"
		click_on "Crear nuevo estudiante"
		
			fill_in "Nombre(s)", with: "Edwin"
			fill_in "Apellidos", with: "Harrizon"
			fill_in "Número de control", with: "10460316"
			fill_in "Semestre", with: "11"

		click_on "Guardar"
		click_on "Editar"

			fill_in "Apellidos", with: "Serna"
		click_on "Guardar"
		sleep 5

			expect(current_path).to eq "/"
			expect(page).to have_content "Student was successfully updated."
		
	end
end